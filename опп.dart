...
class CreditCard implements PaymentSystem {
  ...
  @override
  void refund(double amount) {
    print('Refunded $amount using Credit Card.');
  }
}
...
class PayPal implements PaymentSystem {
  ...
  @override
  void refund(double amount) {
    print('Refunded $amount using PayPal.');
  }
}
...
class RectangleShape {
  ...
  RectangleShape(this.topLeft, this.bottomRight);
...
class ProductOrder {
  ...
  ProductOrder(this.name, this.price, this.quantity);
}
...
class Plaayyer {
  ...
  Plaayyer(this.name, this.symbol);
}

class Gaame {
  List<List<String>> board;
  Plaayyer currentPlayer;
  Plaayyer player1;
  Plaayyer player2;

  Gaame(this.player1, this.player2)
  ...
  void makeMove(int row, int col) {
    if (board[row][col] == '') {
      ...
      if (checkWin()) {
        ...
        printBoard();
        exit(0); // Завершаем игру при победе
      } else if (isBoardFull()) {
        print('It\'s a draw!');
        printBoard();
        exit(0); // Завершаем игру при ничьей
      } else {
...
void main() {
  ...
  //
  // Store store = Store();
  // store.addProduct(Product('Laptop', 1000, 1));
  // store.addProduct(Product('Mouse', 50, 10));
  // print(store.findProductByName('Laptop'));
  //
  ...
  // print('ID1: ${id1.id}, ID2: ${id2.id}');
  //
  // Point topLeft = Point(0, 1);
  // Point bottomRight = Point(2, 0);
  // RectangleShape rect = RectangleShape(topLeft, bottomRight);
  // print('Rectangle Area: ${rect.getArea()}');
  //
  // ComplexNumber c1 = ComplexNumber(3, 4);
  // ComplexNumber c2 = ComplexNumber(1, 2);
  // print('Add: ${c1.add(c2)}');
  // print('Subtract: ${c1.subtract(c2)}');
  // print('Multiply: ${c1.multiply(c2)}');
  // print('Divide: ${c1.divide(c2)}');
  //
  ...
  // Order order = Order();
  // order.addProduct(ProductOrder('Laptop', 1000, 1));
  ...
  // Plaayyer player1 = Plaayyer('Player 1', 'X');
  // Plaayyer player2 = Plaayyer('Player 2', 'O');
  // Gaame game = Gaame(player1, player2);
  //
  // while (true) {
  //   game.printBoard();
  //   print('${game.currentPlayer.name}, enter your move (row and column): ');
